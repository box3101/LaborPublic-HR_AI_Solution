...
export default {
  ...
  methods: {
    ...
    connectWebSocket() {
      ...
      // 환경 변수에서 HRD WebSocket URL 가져오기
      const wsUrl = config.apiBaseUrl+'/ws/hrd';
      console.log(`[${config.env}] HRD WebSocket 연결: ${wsUrl}`);
      
      this.socket = new WebSocket(`${wsUrl}?token=${Cookies.get('token')}`);
